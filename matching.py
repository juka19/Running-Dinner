...
for hoster in list(set(df['Team'][df['course']=='starters'])):
    guests=(random.sample(starter_guests_main, 1)[0], 
    random.sample(starter_guests_dessert, 1)[0], 
    hoster)
    starter_guests_main.remove(guests[0])
    starter_guests_dessert.remove(guests[1])
...
for hoster in list(set(df['Team'][df['course']=='dessert'])):
    ...
    fdgm=[guest for guest in (list(set(df['Team'][df['starters_id']!=hoster_id].loc[df.Team.isin(dessert_guests_main)]))&
                              list(set(df['Team'][df['main_id']!=hoster_id].loc[df.Team.isin(dessert_guests_main)])))]
    fdgs=[guest for guest in (list(set(df['Team'][df['starters_id']!=hoster_id].loc[df.Team.isin(dessert_guests_starter)])) &
                              list(set(df['Team'][df['main_id']!=hoster_id].loc[df.Team.isin(dessert_guests_starter)]))]
    guests=(fdgs.pop(0), fdgm.pop(0), hoster)
    main_guests_starter.remove(guests[0])
    main_guests_main.remove(guests[1])
